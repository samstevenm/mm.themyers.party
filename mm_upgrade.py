...
class SSHtoMMServerStep(object):
    def run(self, context):
        print("**SSH into Mattermost Server (Ubuntu)**")
        print("Run:\n")
...
class PrepUbuntuStep(object):
    def run(self, context):
        print("**Completely update Mattermost Server (Ubuntu)**")
        print("Run:\n")
        print("")
        print("   sudo apt update && sudo apt upgrade -y")
        wait_for_enter()

        print("**Remove unnecessary stuff from Mattermost Server (Ubuntu)**")
        print("Run:\n")
        print("   sudo apt-get autoremove --purge -y")
        wait_for_enter()

        print("**Free up space by vacuuming the logs (Ubuntu)**")
        print("Run:\n")
        print("   sudo journalctl --vacuum-time=3d")
        wait_for_enter()
        print("**Make a note of disk usage (Ubuntu)**")
        print("Run:\n")
        print("   df -h")
        print("\n")
...
class DumpMMSQLStep(object):
    def run(self, context):
        print("**Dump the mattermost database to a SQL file**")
        print("This next command will prompt you for your root password.")
        print("Run:")
        print("\n")
        #There are escaped (")s - (\") in the printed line. This is important!
        # The actual command is: mysqldump -u root -p mattermost > mattermost_backup$(date +"%Y-%m-%d_%H-%M-%S").sql
        print("   mysqldump -u root -p mattermost > mattermost_backup$(date +\"%Y-%m-%d_%H-%M-%S\").sql")
        wait_for_enter()
...
If you do not know where Mattermost Server is installed, use the
`whereis mattermost` command. The output should be similar to
/opt/mattermost, /opt/mattermost/bin/mattermost/ or /bin/mattermost.
...
class ChangeDirStep(object):
    def run(self, context):
        print('''
In a terminal window on the server that hosts Mattermost, change to your /tmp
directory. Delete or rename any old Mattermost files and directories.
        ''')
        print("Run:\n")
        print("cd /tmp")
        wait_for_enter()
...
class ConfirmDirEmpty(object):
    def run(self, context):
        print('''
Confirm no other Mattermost zip folders exist in your /tmp directory. If other
versions or files exist, delete or rename them.
              ''')
        print("Run:\n")
...
class ExtractMMServerFilesStep(object):
    def run(self, context):
        print("Run:\n")
        #There is an escaped (\) - (\\) in the next line. This is important!
        #The actual command is: tar -xf mattermost*.gz --transform='s,^[^/]\+,\0-upgrade,'
